...
| `AUTH_COOKIE_SECURE`          | Cookie `Secure` flag. Forced **true** when `NODE_ENV=production`; set `false` only outside production.                                                                                           |
| `APP_USERNAME`                | Staff login user. No default.                                                                                                                                                                    |
| `APP_PASSWORD`                | **Required in production** when `AUTH_ENABLED=true` (default). ≥12 characters. Server **exits on start** if missing or too short.                                                               |
| `SESSION_SECRET`              | **Required in production** (and whenever `AUTH_ENABLED=true`). ≥32 characters. HMAC key for signed sessions and HKDF input for `jira-oauth.json` at rest. Server **exits on start** if missing or too short. |
| `STAFF_SESSION_VERSION`       | Optional. Default `1`. Bump to invalidate staff sessions after rotating `APP_PASSWORD`.                                                                                                          |
| `DEBUG_ENDPOINTS`             | Optional. Default off. Set `true` to enable staff-only `/api/debug/*` (deliverable diagnostics). Leave unset in production unless you are debugging a specific run. |
| `TRUST_PROXY_HOPS`            | Optional. Default `1` (Caddy). Express `trust proxy` hop count.                                                                                                                                  |
...
| `TURNSTILE_SEND_IP` | Optional. Default off. Set `true` to send the client IP to Turnstile. |
| `AUTH_EMAIL_VERIFY` | Optional. Unset: `required` in production, `auto` otherwise. |
| `LEAD_RETENTION_DAYS` | Optional. Default `365`. |
